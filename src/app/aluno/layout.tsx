...
import { clearAuthData, getUserName, getUserRole, isFirstLogin } from '@/lib/auth';
import { apiFetch } from '@/lib/api';
import { restoreTheme, toggleTheme } from '@/lib/theme';

const navItems = [
  { name: 'Catálogo', href: '/aluno/dashboard', icon: BookOpen },
  { name: 'Minhas Reservas', href: '/aluno/reservations', icon: CalendarClock },
  { name: 'Meu Perfil', href: '/aluno/profile', icon: User },
];
...
export default function StudentLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      else if (isFirstLogin()) router.replace('/primeiro-acesso');
    }, 0);
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur dark:border-slate-800 dark:bg-slate-900/90">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            <div className="hidden min-w-48 items-center gap-2 rounded-xl bg-slate-100 px-4 py-2 text-sm font-medium text-slate-700 dark:bg-slate-800 dark:text-slate-200 lg:flex">
