...
import { clearAuthData, getUserEmail, getUserName, getUserRole, isFirstLogin } from "@/lib/auth";
import { apiFetch } from "@/lib/api";
import { restoreTheme, toggleTheme } from "@/lib/theme";

const navItems = [
  { name: "Dashboard", href: "/admin/dashboard", icon: LayoutDashboard },
  { name: "Livros", href: "/admin/books", icon: BookOpen },
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-900 dark:text-slate-100">
      ...
      <div className="md:pl-72">
        ...
        <main className="w-full min-w-0 p-4 sm:p-6 md:p-8 xl:p-10">{children}</main>
      </div>
    </div>
  );
}
