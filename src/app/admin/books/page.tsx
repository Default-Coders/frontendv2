...
export default function AdminBooksPage() {
  ...
  useEscapeKey(() => {
    if (stockBook) setStockBook(null);
  ...
  async function loadData() {
    try {
      ...
      setFeedback({
        type: "error",
        text: err instanceof Error ? err.message : "Erro ao carregar livros.",
      });
    } finally {
      setLoading(false);
    }
  ...
  async function handleCreate(e: FormEvent) {
    ...
    setSaving(true);
    try {
      const created = await apiFetch<Book>("/livros", {
        method: "POST",
        body: JSON.stringify({
          title,
          author,
      ...
      setFeedback({ type: "success", text: "Livro cadastrado com sucesso!" });
      setShowCreateModal(false);
      await loadData();
    } catch (err) {
      setFeedback({
        type: "error",
        text: err instanceof Error ? err.message : "Erro ao cadastrar livro.",
      });
    } finally {
      setSaving(false);
    }
  ...
  async function handleEdit(e: FormEvent) {
    ...
    setSaving(true);
    try {
      await apiFetch(`/livros/${editingBook.id}`, {
        method: "PUT",
        body: JSON.stringify({
          title,
          author,
      ...
      setFeedback({
        type: "error",
        text: err instanceof Error ? err.message : "Erro ao atualizar livro.",
      });
    } finally {
      setSaving(false);
    }
  ...
  async function uploadCover(bookId: string, file: File) {
    ...
    await apiFetch(`/livros/${bookId}/cover`, {
      method: "POST",
      body: formData,
    });
  }
  ...
  async function handleRemoveCover() {
    if (
      ...
      !(await confirmToast({ message: "Deseja remover a capa deste livro?", confirmLabel: "Remover" }))
    )
      return;
    setSaving(true);
    try {
      await apiFetch(`/livros/${editingBook.id}/cover`, { method: "DELETE" });
  ...
  async function handleUpdateStock(e: FormEvent) {
    ...
    setSaving(true);
    try {
      await apiFetch(`/livros/${stockBook.id}/stock`, {
        method: "PATCH",
        body: JSON.stringify({ newTotalQuantity: newStock }),
      });
      setFeedback({ type: "success", text: "Estoque atualizado com sucesso!" });
      setStockBook(null);
      await loadData();
    } catch (err) {
      setFeedback({
        type: "error",
        text: err instanceof Error ? err.message : "Erro ao ajustar estoque.",
      });
    } finally {
      setSaving(false);
    }
  ...
  async function handleDelete(id: string, bookTitle: string) {
    if (!(await confirmToast({ message: `Deseja desativar o livro “${bookTitle}”?`, confirmLabel: "Desativar" }))) return;
    setFeedback(null);
    try {
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <Link href={`/livros/${book.id}`} className="font-bold text-slate-900 hover:text-blue-600 hover:underline dark:text-white dark:hover:text-blue-400">
                            {book.title}
                          </Link>
