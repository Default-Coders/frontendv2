...
interface CreatedAdministrator extends Administrator {
...
export default function AdministratorsPage() {
  ...
  useEscapeKey(() => {
    ...
    else if (resetting) setResetting(null);
  ...
  const loadAdmins = useCallback(async () => {
    ...
    try {
      ...
      setAdmins(await apiFetch<Administrator[]>(`/administradores?${params}`));
    } catch (error) {
  ...
  function openCreate() {
    setName(""); setEmail(""); setCreating(true);
  }
  ...
  async function createAdmin(event: FormEvent) {
    ...
    try {
      const created = await apiFetch<CreatedAdministrator>("/administradores", { method: "POST", body: JSON.stringify({ name, email }) });
      setTemporaryCredential({ name: created.name, password: created.temporaryPassword, emailSent: created.emailSent });
      toast.success("Administrador cadastrado com senha temporária.");
  ...
  async function updateAdmin(event: FormEvent) {
    ...
    try {
      await apiFetch(`/administradores/${editing.id}`, { method: "PUT", body: JSON.stringify({ name, email }) });
  ...
  async function resetPassword(event: FormEvent) {
    ...
    try {
      const result = await apiFetch<{ emailSent: boolean }>(`/administradores/${resetting.id}/password`, { method: "PATCH", body: JSON.stringify({ newPassword: password }) });
      toast.success(result.emailSent ? `Senha de “${resetting.name}” redefinida e enviada por e-mail.` : `Senha redefinida, mas o e-mail não foi enviado. Entregue-a por outro canal seguro.`);
  ...
  async function changeStatus(admin: Administrator) {
    ...
    if (!(await confirmToast({
      message: `Deseja ${action} o administrador “${admin.name}”?`,
      confirmLabel: admin.active ? "Desativar" : "Reativar",
      danger: admin.active,
    }))) return;
    try {
      await apiFetch(`/administradores/${admin.id}${admin.active ? "" : "/reactivate"}`, { method: admin.active ? "DELETE" : "PATCH" });
  ...
  return (
    <div className="space-y-6">
      ...
      {(creating || editing) && <Modal title={creating ? "Cadastrar Administrador" : "Editar Administrador"} close={() => creating ? setCreating(false) : setEditing(null)}><form onSubmit={creating ? createAdmin : updateAdmin} className="space-y-4"><Field label="Nome" value={name} setValue={setName} /><Field label="E-mail" type="email" value={email} setValue={setEmail} />{creating && <p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-800 dark:bg-amber-950/40 dark:text-amber-200">Uma senha temporária aleatória será exibida uma única vez após o cadastro.</p>}<Actions saving={saving} cancel={() => creating ? setCreating(false) : setEditing(null)} /></form></Modal>}
      ...
      {temporaryCredential && <TemporaryPasswordModal userName={temporaryCredential.name} password={temporaryCredential.password} emailSent={temporaryCredential.emailSent} onClose={() => setTemporaryCredential(null)} />}
    </div>
  );
}
